...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        return f"{mission}_{source}_chunk_{int(chunk_index):04d}"
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        # Replace mode: delete existing docs from this source
        ...
        for text, meta in documents:
            ...
            if exists:
                if update_mode == 'skip':
                    ...
                    try:
                        self.collection.update(ids=[doc_id], documents=[text], metadatas=[meta], embeddings=[emb])
                        stats['updated'] += 1
                    except Exception:
            ...
            if len(batch_ids) >= batch_size:
                try:
                    self.collection.add(ids=batch_ids, documents=batch_docs, metadatas=batch_metas, embeddings=batch_embs)
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                add_stats = self.add_documents_to_collection(chunks, file_path, batch_size=50, update_mode=update_mode)
                stats['documents_added'] += add_stats.get('added', 0)
                stats['documents_updated'] += add_stats.get('updated', 0)
                stats['documents_skipped'] += add_stats.get('skipped', 0)
                ...
                stats['missions'][mission]['added'] += add_stats.get('added', 0)
                stats['missions'][mission]['updated'] += add_stats.get('updated', 0)
                stats['missions'][mission]['skipped'] += add_stats.get('skipped', 0)
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'document_count': len(all_docs.get('ids', [])),
                'example_metadata': all_docs.get('metadatas', [])[:3]
            }
        except Exception as e:
            return {'error': str(e)}
    ...
    def query_collection(self, query_text: str, n_results: int = 5) -> Dict[str, Any]:
        ...
        """
        try:
            res = self.collection.query(query_texts=[query_text], n_results=n_results, include=["metadatas", "documents", "distances"])
            return res
        except Exception as e:
