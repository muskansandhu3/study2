...
def sample_images(
    ...
    if class_labels is None:
        labels = torch.tensor([i % num_classes for i in range(num_samples)], device=device)
    else:
...
def train_diffusion(
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % save_every == 0:
            save_checkpoint(model, optimizer, epoch + 1, loss.item(), name="diffusion_unet", path=checkpoint_dir)

    save_checkpoint(model, optimizer, epoch="final", loss=loss.item(), name="diffusion_unet", path=checkpoint_dir)
