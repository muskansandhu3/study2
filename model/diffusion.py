...
class ConditionalUNet(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        h = self.mid(h3, t_emb, y)
        h = self.up3(torch.cat([F.interpolate(h, scale_factor=2, mode="nearest"), h2], dim=1), t_emb, y)
        h = self.up2(torch.cat([F.interpolate(h, scale_factor=2, mode="nearest"), h1], dim=1), t_emb, y)
